...
export default function Sales3Page() {
  ...
  const allFilteredCvs = useMemo(() => {
    ...
    return cvs.filter(cv => {
      ...
      // فلتر الدين - يعمل مع قاعدة البيانات
      const matchesReligion = religionFilter === 'ALL' || (() => {
        if (!cv.religion) return false
        if (religionFilter === 'مسلمة') {
          return cv.religion.includes('مسلم') || cv.religion.includes('MUSLIM')
        }
        if (religionFilter === 'مسيحية') {
          return cv.religion.includes('مسيحي') || cv.religion.includes('CHRISTIAN')
        }
        if (religionFilter === 'أخرى') {
          return !cv.religion.includes('مسلم') && !cv.religion.includes('مسيحي')
        }
        return false
      })()
      ...
      return matchesSearch && matchesStatus && matchesPosition && matchesNationality && 
             excludeDriversFromNationality && matchesMaritalStatus && 
             matchesSkill && matchesArabicLevel && matchesEnglishLevel && 
             matchesEducation && matchesExperience && matchesReligion &&
             matchesHeight && matchesWeight
    })
  }, [cvs, searchTerm, statusFilter, nationalityFilter, positionFilter, maritalStatusFilter, 
      skillFilters, arabicLevelFilter, englishLevelFilter, 
      religionFilter, educationFilter, experienceFilter, heightFilter, weightFilter, matchesNationalityFilter])
      
  // قائمة مفلترة للعرض (تحديد عدد معين للعرض)
...
إذا متوفر علمّوني الله يعطيكم العافية `;
  ...
  const shareSingleCV = async (cv: CV) => {
    const shareUrl = `${window.location.origin}/cv/${cv.id}?from=sales3`
        ...
        <div className="bg-white rounded-lg shadow-md border border-gray-200 p-4 sm:p-6 mb-6">
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showAdvancedFilters ? 'max-h-[1000px] opacity-100 mb-6' : 'max-h-0 opacity-0'}`}>
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                ...
                <div className="space-y-2">
                  <label className="flex items-center text-sm font-semibold text-orange-600 mb-2">
                    <BookOpen className="h-4 w-4 ml-2" /> الحالة الاجتماعية
                  </label>
                  <select
              ...
              {/* صف للفلاتر الجديدة: الطول والوزن والمنطقة */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                <div className="space-y-2">
                  <label className="flex items-center text-sm font-semibold text-teal-600 mb-2">
                    <BookOpen className="h-4 w-4 ml-2" /> الطول
                  </label>
                  <select
