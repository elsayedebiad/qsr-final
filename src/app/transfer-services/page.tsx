...
export default function TransferServicesPage() {
  ...
  const [positionFilter, setPositionFilter] = useState<string>('نقل خدمات') // فلتر تلقائي لنقل خدمات
  ...
  // تم إزالة جميع أكواد البنرات من هذه الصفحة

  // جلب رقم الواتساب والإعدادات المخصصة
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setHideFilters(data.hideFilters || false)
        }
      } catch (error) {
...
هذا رابط سيرته: ${window.location.origin}/cv/${cv.id}?from=transfer-services
...
إذا متوفر علمّوني الله يعطيكم العافية `;
  ...
  const shareSingleCV = async (cv: CV) => {
    const shareUrl = `${window.location.origin}/cv/${cv.id}?from=transfer-services`
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl font-semibold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent truncate">
                  معرض نقل الخدمات
                </h1>
                <p className="text-gray-600 text-xs sm:text-sm hidden sm:block">عرض السير الذاتية لوظيفة نقل خدمات</p>
              </div>
            </div>
