...
export default function Sales8Page() {
...
هذا رابط سيرته: ${window.location.origin}/cv/${cv.id}?from=sales8
...
إذا متوفر علمّوني الله يعطيكم العافية `;
  ...
  const shareSingleCV = async (cv: CV) => {
    const shareUrl = `${window.location.origin}/cv/${cv.id}?from=sales8`
